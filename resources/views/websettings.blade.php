    ...
    <div class="la-dashboard-v2">
        ...
        <section class="card">
            <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
                <div>
                    <h5 class="card-title mb-1">{{ __('Website settings') }}</h5>
                </div>
            </div>
            <div class="card-body">
                @if (session('status'))
                ...
                <form method="POST" action="{{ route('websettings.update') }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-3">
                        <div class="col-md">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                                name="name" value="{{ old('name', $settings->name) }}" required>
                            @error('name')
                        ...
                        @if ($ranks->isNotEmpty())
                            <div class="col-12">
                                <label class="form-label">{{ __('Ranks') }}</label>
                                <ul class="list-group">
                                    @foreach ($ranks as $rank)
                                        <li class="list-group-item d-flex justify-content-between align-items-center">
                                            {{ $rank->name }}
                                            <span class="badge bg-secondary">{{ $rank->rank_id }}</span>
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
                        ...
                        <div class="col-12" id="ranksWrap">
                            <div class="rank-field mb-2">
                                <label for="rank-0" class="form-label">{{ __('New rank') }}</label>
                                <input type="text" class="form-control rank-input" id="rank-0" name="ranks[]">
                            </div>
                        </div>
...
@push('scripts')
    <script>
        (function() {
            'use strict';
            const ranksWrap = document.getElementById('ranksWrap');
            if (!ranksWrap) return;

            let rankCount = ranksWrap.querySelectorAll('.rank-field').length;

            function addRankField() {
                const index = rankCount++;
                const field = document.createElement('div');
                field.className = 'rank-field mb-2';
                field.innerHTML = `
                    <label for="rank-${index}" class="form-label">{{ __('New rank') }}</label>
                    <input type="text" class="form-control rank-input" id="rank-${index}" name="ranks[]">
                `;
                ranksWrap.appendChild(field);
            }

            function removeRankFieldsAfter(field) {
            ...
            ranksWrap.addEventListener('input', function(event) {
                const input = event.target;
                if (!input.classList.contains('rank-input')) return;

                const field = input.closest('.rank-field');
                const isLast = field === ranksWrap.querySelector('.rank-field:last-child');

                if (input.value.trim() !== '' && isLast) {
                    addRankField();
                } else if (input.value.trim() === '' && !isLast) {
                    removeRankFieldsAfter(field);
                }
            });
        })();
